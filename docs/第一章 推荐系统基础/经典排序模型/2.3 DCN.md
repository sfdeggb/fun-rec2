...
2017年,斯坦福大学和谷歌的研究人员在ADKDD会议上提出了Deep&Cross模型(DCN)。该模型是在Wide&Deep模型的基础上进行改进的一个典型代表。Wide&Deep模型通过结合"记忆能力"和"泛化能力",开创了融合不同网络结构的新思路。但是Wide&Deep模型的Wide部分存在一个明显的缺陷 - 需要人工进行特征组合的筛选,这个过程不仅繁琐而且需要大量经验。
虽然FM模型能够在线性时间复杂度内自动进行特征交互,但是其特征交互的表达能力有限。并且当特征交互的阶数增加时,模型的复杂度会急剧上升。为了解决这些问题,DCN模型使用Cross Network替代了Wide部分。Cross Network可以自动进行特征之间的交叉,同时保持线性的时间和空间复杂度。最终,Cross Network与Deep部分相结合,构成了深度交叉网络(Deep & Cross Network)。

## 2. 模型结构及原理

这个模型的结构是这个样子的：
<p align="center">
<img src="../../../assest/DCN.png" style="zoom:30%;" />
</p>
...
<p align="center">
<img src="../../../assest/DCN_Cross.png" style="zoom:40%;" />
</p>
...
下面我们看下deepctr库中的DCN的代码复现，这里主要是给大家说一下这个模型的设计逻辑，参考了deepctr的函数API的编程风格， 具体的代码以及示例大家可以去参考后面的GitHub，里面已经给出了详细的注释， 这里主要分析模型的逻辑这块。关于函数API的编程式风格，我们还给出了一份文档， 大家可以先看这个，再看后面的代码部分，会更加舒服些。
