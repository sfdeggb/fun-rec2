...
关于GBDT的详细细节，依然是可以参考下面给出的链接。
这里想分析一下GBDT如何来进行二分类的，因为我们要明确一点就是**GBDT 每轮的训练是在上一轮的训练的残差基础之上进行训练的**， 而这里的残差指的就是当前模型的负梯度值， 这个就要求每轮迭代的时候，弱分类器的输出的结果相减是有意义的， 而**GBDT 无论用于分类还是回归一直都是使用的CART 回归树**， 那么既然是回归树， 是如何进行二分类问题的呢？ 
...
<img src="https://img-blog.csdnimg.cn/20200910095539432.png#pic_center" alt="在这里插入图片描述" style="zoom:80%;" /> 
 	   ...
 	   我们希望构建 GBDT 分类树，它能通过「喜欢爆米花」、「年龄」和「颜色偏好」这 3 个特征来预测某一个样本是否喜欢看电影。			
       我们把数据代入上面的公式中求Loss:
$$
...
再比如举一个GBDT解决回归问题的例子。
假设假定有4个人 (A,B,C,D)，他们的年龄分别是 (14,16,24,26)。其中，A、B分别是高一和高三学生；C、D分别是应届毕业生和工作两年的员工。现在给一个人D样本，我们要预测这个人的年龄。
由于样本数据少，我们限定叶子节点最多为2（即每棵树都只有一个分枝），并且限定树的棵树为2。首先按照购物金额的条件将样本进行划分，得到的结果如图所示。
![GBDT_回归](../../../assest/gbdt1.png)
从上图中可以看出：A、B年龄较为相近，C、D年龄较为相近，被分为左右两支，每支用平均年龄作为预测值。
我们计算残差（即**实际值-预测值**），所以A的残差14-15=-1。同理可得B的残差为1，C的残差为-1，D的残差为1。
接下来我们再对残差进行学习：
![GBDT_回归_2](../../../assest/gbdt2.png)
* 把A、B、C、D的值换作残差-1、1、-1、1，再构建一棵树学习，这棵树只有两个值1和-1，直接分成两个节点：A、C在左边，B、D在右边。从图中可以看到，在我们当前这个简单的场景下，已经能保证预测值和实际值（上一轮残差）相等了。
* 把这棵树的预测值累加到第一棵树上的预测结果上，就能得到真实年龄，这个简单例子中每个人都完美匹配，得到了真实的预测值。
![GBDT_回归_3](../../../assest/gbdt3.png)
...
2014年， Facebook提出了一种利用GBDT自动进行特征筛选和组合， 进而生成新的离散特征向量， 再把该特征向量当做LR模型的输入， 来产生最后的预测结果， 这就是著名的GBDT+LR模型了。GBDT+LR 使用最广泛的场景是CTR（Click through Rating）点击率预估，即预测当给用户推送的广告会不会被用户点击。
...
4. 在CRT预估中， GBDT一般会建立两类树(非ID特征建一类， ID类特征建一类)， AD，ID类特征在CTR（Click through Rating）预估中是非常重要的特征，直接将AD，ID作为feature进行建树不可行，故考虑为每个AD，ID建GBDT树。
...
随机森林是一种Bagging方法，它通过构建多棵决策树并取其模式的平均值或多数投票来提高模型的稳定性和准确性。随机森林的主要优点在于减少方差，通过并行训练多棵树来实现，而GBDT则是通过逐步添加树来减少偏差，这种方法在某些情况下可以提供更高的预测精度。
2. **面对高维稀疏类特征的时候(比如ID类特征)， 逻辑回归一般要比GBDT这种非线性模型好， 为什么？** 
面对高维稀疏特征，比如很多ID类特征，逻辑回归通常比GBDT这类非线性模型更好用，因为逻辑回归简单直接，不容易在这些复杂数据上过拟合，而且计算起来更快，而GBDT在这些数据上可能会因为特征太多太复杂而变得效率低下，还容易学得太深，导致模型在实际应用中表现不好。
